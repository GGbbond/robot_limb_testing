...
如果 FPGA 存在，但 `hardware_elf3` 因所需电机未接全、CAN 口连接错误或
初始化失败而异常退出，统一启动脚本会停止实机控制、自动回退到 MuJoCo
仿真并弹出原因提示，不再直接关闭软件。正常关闭界面或主动切换模式不会触发
自动回退。
...
所有数值参数框都忽略鼠标滚轮，在左侧面板上滚动时不会误改参数；仍可使用
键盘输入或输入框右侧的加减按钮调整。
