...
速度安全保护使用持续超限判定，避免 MuJoCo 单个反馈采样的瞬时尖峰造成误停：仿真速度连续超限 `50 ms` 才锁定，实机连续超限 `10 ms` 即锁定。该滤波只忽略短促尖峰，持续超速仍会立即停止发送控制命令。
...
调试模式中保存的检测对象、运动参数和合格阈值会被生产模式直接使用。界面会记住上次选择的模式；首次运行默认使用生产模式。生产模式不会绕过安全确认、反馈超时、命令间隔、关节限位、速度、力矩或碰撞保护。
...
MuJoCo 仿真模式用于调试，界面参数上限统一放宽到 `1,000,000,000`，循环次数上限为 `2,147,483,647`，可视为不限制上限。实机模式仍使用下列安全范围。无穷值、非数值、负时间、除零参数以及关节软件限位不会被允许，碰撞预测和脚底地面保护也始终生效。
