...
实机台架不要求 29 个身体电机全部连接。启动硬件驱动时，软件根据共享
`settings.json` 中保存的“部位 + 台架侧”自动生成 `motor_disable`，只启用本次
所选手臂或腿的电机。腰部、头部和其他未选肢体会在驱动在线检查之前屏蔽，
所以它们没有连接属于正常情况。

| 保存的检测对象 | 要求在线的电机 | `motor_disable` |
|---|---:|---:|
| 左臂 | 7 | `0x7FC07FFF` |
| 右臂 | 7 | `0x603FFFFF` |
| 双臂 | 14 | `0x60007FFF` |
...
请在仿真界面选好当前实际接入的肢体，再切换到实机模式；模式切换会先保存
当前选择，然后按该选择启动硬件驱动。手动直接执行 `ros2 launch` 时可用
`motor_disable_mask:=<十进制掩码>` 显式覆盖。
