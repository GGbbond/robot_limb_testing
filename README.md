...
才检查数值能否形成有效轨迹。建议先用默认值完成单侧小幅测试，再逐项调整。

| 参数 | 单位 | 作用与调整影响 |
| --- | --- | --- |
| 部位 | — | 选择手臂或腿部关节集合。 |
| 台架侧 | — | 选择左侧、右侧或左右对应关节同步运动。实机首次测试建议单侧。 |
| 检测模式 | — | “小幅往复”围绕初始化位置运动；“安全全行程”按碰撞和机械限位计算绝对目标。 |
| 单向幅度 | ° | 仅用于小幅往复，表示初始化位置两侧的角度幅度。增大会扩大运动范围。 |
| 最短单程时间 | s | 小幅模式中是一段单向轨迹的时间；全行程模式中是最短时间，软件可能为满足速度上限自动延长。 |
...
| 正反循环 | 次 | 仅用于小幅往复；一次包含正向、回中、反向、回中。 |
