...
# Conservative software position limits copied from data/elf3.xml. The hardware
# driver still provides its own independent position, speed and torque limits.
...
class VibrationTestNode(Node):
    ...
    def __init__(self):
        ...
        self.publisher_watchdog_timer = None
    ...
    def _validate_parameters(self):
        ...
        if self.hardware_mode:
            ...
            if not self.joint_test_required:
                raise ValueError(
                    "hardware_mode requires joint_test_required=true"
                )
    ...
    def _motion_callback(self, msg):
        ...
        with self.state_lock:
            if (
                self.previous_btn_9 is None
                or (
                    self.last_motion_command_at > 0.0
                    and now - self.last_motion_command_at
                    > self.motion_command_resync_sec
                )
            ):
                self.previous_btn_9 = current
                self.last_motion_command_at = now
                return
            self.last_motion_command_at = now
            if self.motion_button_mode == "toggle":
                # The C++ remote_controller publishes btn_9 as a persistent
                # toggle flag, so either value change is one physical X press.
                activated = current != self.previous_btn_9
            else:
                # Keyboard/momentary sources publish 1 while held and 0 on
                # release; only the rising edge is an action.
                activated = current and not self.previous_btn_9
            self.previous_btn_9 = current
    ...
    def _timer_callback(self):
        ...
        with self.state_lock:
            ...
            if self._publish_command(command, JOINT_KP, JOINT_KD) and log_sample:
    ...
    def _envelope_violation_details(self, center, amplitude, indices):
    ...
    def _begin_smooth_return_locked(self, owner):
    ...
    def _return_to_center_command(self, now):
        with self.state_lock:
            progress = (now - self.return_started_at) / self.stop_ramp_sec
